...
const makeActiveRun = async (acceptFollowUps: boolean): Promise<Pick<ActiveRun, "followUpQueue" | "acceptFollowUps">> => ({
...
const drainQueue = <A>(queue: Queue.Queue<A>) => Effect.runPromise(Queue.takeAll(queue).pipe(Effect.map((items) => [...items])))
...
describe("enqueueRunRequest", () => {
  ...
  test("routes to the follow-up queue when the active run accepts follow-ups", async () => {
    ...
    expect((await drainQueue(activeRun.followUpQueue)).map((item) => item.message.id)).toEqual(["m-1"])
  ...
  test("reads the live follow-up gate on each call", async () => {
    ...
    expect(await Effect.runPromise(enqueueRunRequest(session, makeRequest("m-1")))).toBe("follow-up")
