...
import { SessionStore, type SessionStoreShape } from "@/sessions/store.ts";
...
type CreateLoadedSessionInput = { workdir: string; title: string; systemPromptAppend?: string };
type HarnessOptions = {
  createOpencodeSession?: (
    input: CreateLoadedSessionInput,
...
const makeHarness = async (options?: HarnessOptions) => {
  const loadedSessions = await runTestEffect(buildLoadedSessionDirectory());
  const created = await makeRef<CreateLoadedSessionInput[]>([]);
