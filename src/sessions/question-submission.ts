...
import { setQuestionBatchStatus, type QuestionBatchState } from "@/sessions/question-batch-state.ts"
...
type QuestionSubmissionRuntime<Batch extends QuestionSubmissionBatch<SessionHandle>, Interaction, SessionHandle> = {
  persistBatch: (
    requestId: string,
...
export const submitQuestionBatch =
  ...
  ({ interaction, requestId, answers }: SubmitQuestionInput<Interaction>): Effect.Effect<boolean, unknown> =>
    Effect.gen(function* () {
      ...
      const submitResult = yield* runtime.submitToOpencode(submitting.session.opencode, submitting.request.id, answers).pipe(
        Effect.either,
      )
...
export const rejectQuestionBatch =
  ...
  ({ interaction, requestId }: RejectQuestionInput<Interaction>): Effect.Effect<boolean, unknown> =>
    Effect.gen(function* () {
      ...
      const rejectResult = yield* runtime.rejectInOpencode(rejecting.session.opencode, rejecting.request.id).pipe(
        Effect.either,
      )
