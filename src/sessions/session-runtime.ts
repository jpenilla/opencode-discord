...
import { type ChannelActivity, type SessionActivity } from "@/sessions/session-activity.ts";
...
import { makeQuestionRuntime, QuestionRuntime } from "@/sessions/question/question-runtime.ts";
...
import { clearRunInterrupt, requestRunInterrupt } from "@/sessions/run/active-run-state.ts";
...
import { SessionStoreLayer } from "@/sessions/session-store.ts";
      ...
      return startShutdown().pipe(
        Effect.flatMap((shouldRun) =>
          !shouldRun
            ? Effect.void
            : Effect.gen(function* () {
                const handles = yield* sessionIndex.snapshotHandles();
