...
const updatePendingPrompt = (
  ...
  Ref.modify(
    stateRef,
    (current): readonly [ReadonlyArray<PromptTrackingAction>, PendingPrompt | null] => {
      if (!current) {
        return [[], null];
      }
      ...
      return [actions, shouldClearPrompt(actions) ? null : nextPrompt];
    },
  );
...
export const beginPendingPrompt = (
  ...
  Effect.gen(function* () {
    ...
    if (existing) {
      throw new Error(
        `Cannot begin prompt: prompt ${existing.userMessageId ?? "<awaiting-user-message>"} is still pending`,
      );
    }
...
export const failPendingPrompt = (
  ...
  Ref.modify(stateRef, (current): readonly [PendingPrompt | null, PendingPrompt | null] => {
    ...
    Effect.flatMap((current) =>
      current ? Deferred.fail(current.deferred, error).pipe(Effect.ignore) : Effect.void,
    ),
  );
