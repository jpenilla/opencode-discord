...
type PromptCoordinatorActiveRun = Pick<ActiveRun, "attachmentMessagesById" | "promptState" | "followUpQueue" | "acceptFollowUps">
  ...
  Effect.gen(function* () {
    const runPrompt = (value: string) =>
      Effect.gen(function* () {
        ...
        yield* input.submitPrompt(input.session, value).pipe(
