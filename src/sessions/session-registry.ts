...
import type { LoadedSessionHandle, LoadedSessionHandleSupport } from "@/sessions/session-handle.ts";
import { makeSessionFactory } from "@/sessions/session-factory.ts";
...
export const makeSessionRegistry = (
  ...
  Effect.gen(function* () {
    ...
    const withExistingOrGatedSession = <A, R>(
      ...
      Effect.gen(function* () {
        const existing = yield* sessionIndex.findLoadedHandle(channelId);
        if (existing) {
        ...
        return yield* withSessionGate(
          channelId,
          Effect.gen(function* () {
            const current = yield* sessionIndex.findLoadedHandle(channelId);
            if (current) {
    ...
    const unloadSession = (handle: LoadedSessionHandle) =>
      sessionIndex.deleteSession(handle).pipe(Effect.andThen(handle.closeResources()));
    ...
    const recreateSession = (session: ChannelSession, message: Message, reason: string) =>
      withSessionGate(
        message.channelId,
        Effect.gen(function* () {
          const currentHandle = yield* sessionIndex.findLoadedHandle(message.channelId);
          if (!currentHandle) {
    ...
    const closeExpiredSessions = (now = Date.now()) =>
      sessionIndex.snapshotHandles().pipe(
    ...
    const invalidateSession = (channelId: string, reason: string) =>
      withSessionGate(
        channelId,
        Effect.gen(function* () {
          const loadedHandle = yield* sessionIndex.findLoadedHandle(channelId);
    ...
    return {
      findLoadedHandle: sessionIndex.findLoadedHandle,
      findLoadedHandleBySessionId: sessionIndex.findLoadedHandleBySessionId,
      findLoadedSession: sessionIndex.findLoadedSession,
      findActiveRunBySessionId,
      findLoadedContextBySessionId: sessionIndex.findLoadedContextBySessionId,
      updateActiveRun,
