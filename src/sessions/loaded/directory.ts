...
type LoadedSessionDirectoryState = {
  handlesByChannelId: Map<string, LoadedSessionHandle>;
  handlesBySessionId: Map<string, LoadedSessionHandle>;
};

export const buildLoadedSessionDirectory = () =>
  Effect.gen(function* () {
    const stateRef = yield* Ref.make<LoadedSessionDirectoryState>({
      handlesByChannelId: new Map(),
      handlesBySessionId: new Map(),
    });

    const read = <A>(map: (state: LoadedSessionDirectoryState) => A) =>
      Ref.get(stateRef).pipe(Effect.map(map));
    const update = (f: (state: LoadedSessionDirectoryState) => void) =>
...
export type LoadedSessionDirectory = Effect.Success<ReturnType<typeof buildLoadedSessionDirectory>>;
