...
} from "@/sessions/session-handle.ts";
import type { SessionIndex } from "@/sessions/session-index.ts";
import type { SessionStoreShape } from "@/sessions/session-store.ts";
import {
...
export const makeSessionFactory = (input: {
    ...
    handle.readSession().pipe(
      Effect.flatMap((session) =>
        Effect.gen(function* () {
          ...
          yield* input.sessionIndex.updateSession(handle);
          yield* handle.startWorker();
          return session;
        }).pipe(
          Effect.onError(() =>
            Effect.gen(function* () {
              yield* input.sessionIndex.deleteSession(handle);
  ...
  const createSessionAt = (sessionInput: CreateSessionInput) =>
    Effect.gen(function* () {
